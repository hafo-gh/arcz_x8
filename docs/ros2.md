...
| --- | --- | --- | --- |
| `arcz_connection` | `mavlink_bridge` | rclpy node | Bridges MAVLink messages to ROS2 topics; message-to-topic mapping is config-driven (`config/mavlink_mappings.yaml`) |
...
Each package builds and runs as its own Docker container (`network_mode: host`, sharing a common `arcz/ros2-base` image), started via that package's own `ros2 launch` file as the container's `CMD`. `docker-compose.yml` at the repo root `include`s every package's own compose file. `scripts/install.sh` builds the base image and generates one systemd unit per package (`docker compose up -d --build` on boot). Crash recovery is layered: ROS2 launch `respawn=True` restarts a dead process, Docker's `restart: unless-stopped` restarts a dead container, systemd brings containers back after a reboot.
